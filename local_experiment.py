...
def run_movinet(frame):
    ...
    # 3. image 입력 quantize
    inp = quantize_input(frame, image_in_detail)
    inp = inp[np.newaxis, np.newaxis, ...]  # (1,1,224,224,3)
    mov_interp.set_tensor(image_in_idx, inp)

    # 4. state 입력
    if movinet_prev_state is None:
        movinet_prev_state = init_state_dict(state_in_details)
    for d in state_in_details:
        mov_interp.set_tensor(d['index'], movinet_prev_state[d['name']])
    mov_interp.invoke()

    # 5. output (dequantize 필요 시)
    out = mov_interp.get_tensor(logits_out_idx)
    # output quantization 정보
    scale, zp = logits_out_detail['quantization']
    if scale != 0:
    ...
    #top5 = get_top_p(probs, k=5, label_map=label_map)

    idx_ = int(probs.argmax())

    # 6. state output 저장
    next_state = {}
    for d in state_in_details:
        if d['name'] in state_out_name_to_idx:
            next_state[d['name']] = mov_interp.get_tensor(state_out_name_to_idx[d['name']])
        else:
            next_state[d['name']] = movinet_prev_state[d['name']]
    movinet_prev_state = next_state

    return f"{label_map[idx_]}", float(probs[idx_])
